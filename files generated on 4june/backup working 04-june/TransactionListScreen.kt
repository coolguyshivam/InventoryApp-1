...
fun TransactionListScreen() {
    ...
    // Load all sales and purchases
    LaunchedEffect(Unit) {
        val saleTask = db.collection("sales").get()
        val purchaseTask = db.collection("purchases").get()

        val salesSnapshot = saleTask.await()
        val purchasesSnapshot = purchaseTask.await()

        val sales = salesSnapshot.documents.mapNotNull { it.toTransactionRecord("Sale") }
        val purchases = purchasesSnapshot.documents.mapNotNull { it.toTransactionRecord("Purchase") }
